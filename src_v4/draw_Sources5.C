...
void draw_Exp(TChain *tExpA, TChain *tExpB, TChain *tInfoA, TChain *tInfoB, 
	const char *name, const char *fname, TCut cXY, TCut cZ)
{
	char str[256];
	long long gtA, gtB;
	double timeA, timeB;
...
void draw_MC(TChain *tMc, const char *name, const char *fname, TCut cXY, TCut cZ, double scale, double RndmSqe, double RndmC)
{
	char str[1024];
	double rAB;
	long NA, NB;
...
void draw_Sources5(int iser, const char *rootdir = "root8n2", double scale = 1.0, double RndmSqe = 0.12, double RndmC = 0.04, int max_files = 0)
{
	const char *name = "";
	char fname[1024];
	char str[1024];
	...
	case 1001:	// Na MC, center
		...
		tMc->AddFile(str);
		name = "22Na";
		sprintf(fname, "22Na_MC_center_%s_S%5.3f_R%5.3f_C%5.3f", rootdir, scale, RndmSqe, RndmC);
		break;
	case 1002:	// Na MC, edge
		...
		tMc->AddFile(str);
		name = "22Na";
		sprintf(fname, "22Na_MC_edge_%s_S%5.3f_R%5.3f_C%5.3f", rootdir, scale, RndmSqe, RndmC);
		break;
	case 1101:	// Co MC, center
		...
		tMc->AddFile(str);
		name = "60Co";
		sprintf(fname, "60Co_MC_center_%s_S%5.3f_R%5.3f_C%5.3f", rootdir, scale, RndmSqe, RndmC);
		break;
	case 1102:	// Co MC, edge
		...
		tMc->AddFile(str);
		name = "60Co";
		sprintf(fname, "60Co_MC_edge_%s_S%5.3f_R%5.3f_C%5.3f", rootdir, scale, RndmSqe, RndmC);
		break;
	default:
		printf("%d - unknown\n", iser);
	...
	switch (code) {
	case 0:	// Experiment
		draw_Exp(tExpA, tExpB, tInfoA, tInfoB, name, fname, cXY, cZ);
		break;
	case 1:	// MC
		draw_MC(tMc, name, fname, cXY, cZ, scale, RndmSqe, RndmC);
		break;
	default:
		break;
	}
...
void sipm2pmt(const char *expname, const char *mcname)
{
	gStyle->SetOptStat(0);
	...
	TH1D *hMCSiPM  = (TH1D *) fMC->Get("hMcSiPM");
	TH1D *hMCPMT   = (TH1D *) fMC->Get("hMcPMT");
	hExpPMT->Scale(hExpSiPM->Integral() / hExpPMT->Integral());
	...
	hExpPMT->SetTitle("Experiment, SiPM and PMT;MeV;N");
	hMCPMT->SetTitle("MC, SiPM and PMT;MeV;N");
