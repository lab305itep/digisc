...
void cm_capture_energy3(const char *fname, const char *mcname, double kRndm = 0.12, double scale = 1.0, double cRndm = 0.04, const char *outname = NULL)
{
	int i, j;
	char str[2048];
	double chi2;
...
//	gStyle->SetPalette(kRainBow, 0);
	...
	sprintf(str, "248Cm_R%4.2f_%4.2f_N%5.3f_FIFRELIN.pdf", kRndm, cRndm, scale);
	...
	printf("%s : %ld\n", str, irc);
	sprintf(str, "MyRandom::GausAdd((SiPmCleanEnergy*1.08+PmtCleanEnergy)/2, %6.4f, %6.4f)", kRndm, cRndm);		// UGLY !
	...
	printf("%s : %ld\n", str, irc);
	sprintf(str, "MyRandom::GausAdd(SiPmCleanEnergy*1.08, %6.4f, %6.4f)", kRndm, cRndm);				// UGLY !
