...
{
	const double ENuMin = 2.0;	// we don't care here much about low energies
...
//		Cuts
	...
	TCut cX("PositronX[0] > 2 && PositronX[0] < 94");
	TCut cY("PositronX[1] > 2 && PositronX[1] < 94");
	TCut cZ("PositronX[2] > 3.5 && PositronX[2] < 95.5");
	TCut cRXY("NeutronX[0] >= 0 && NeutronX[1] >= 0");
	TCut c20("gtDiff > 1");
	TCut cGamma("AnnihilationEnergy < 1.2 && AnnihilationGammas < 12");
	TCut cPe("PositronEnergy > 0.5");
	TCut cPh("PositronHits < 9");
	...
	TCut cN = cNH && cNE;
        TCut ct = c20 && cIso && cVeto && cX && cY && cZ && cR && cPh && cPe && cGamma && cN && cAux;
