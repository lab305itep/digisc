...
//			Create Input chains
	PairChain = new TChain("DanssPair");
//			Add files to input chains
	if (iFirst > 0) {
		...
		for (i = iFirst; i <= iLast; i++) {
			...
			if (i == iStat) {
				sprintf(str, "/home/clusters/rrcmpi/alekseev/igor/pair8n7/%3.3dxxx/pair_%6.6d.root", i / 1000, i);
				PairChain->Add(str);
			}
		}
		fclose(fList);
		...
		PairChain->Add(argv[1]);
	} else {
		printf("Strange argument: .txt, .root or range are expected\n");
		return 30;
	}

	PairChain->SetBranchAddress("Pair", &DanssPair);
	PairChain->SetBranchAddress("PHitE", HitArray[0].E);
	PairChain->SetBranchAddress("PHitT", HitArray[0].T);
	PairChain->SetBranchAddress("PHitType", HitArray[0].type);
	PairChain->SetBranchAddress("NHitE", HitArray[1].E);
	PairChain->SetBranchAddress("NHitT", HitArray[1].T);
	PairChain->SetBranchAddress("NHitType", HitArray[1].type);
	nEvt = PairChain->GetEntries();
	printf("%d events\n",nEvt);
...
//			Open output file
	...
	for (iEvt =0; iEvt < nEvt; iEvt++) {
		PairChain->GetEntry(iEvt);
	// Event cuts
	// muons
		if (DanssPair.SiPmCleanEnergy[0] + DanssPair.PmtCleanEnergy[0] > 40) continue;
		if (DanssPair.SiPmCleanEnergy[1] + DanssPair.PmtCleanEnergy[1] > 40) continue;
		if (DanssPair.gtFromVeto == 0) continue;
	// very soft cuts on NeutronEnergy and distance
		if (DanssPair.NeutronEnergy < 1.5 || DanssPair.Distance > 50) continue;
		if (iEvt < 20) printf("Event No %d: %d and %d hits\n", iEvt, DanssPair.NPHits,  DanssPair.NNHits);
		for (j=0; j<2; j++) {	// prompt and delayed
	// Count SiPM hits in each PMT
			memset(PMTHits, 0, sizeof(PMTHits));
			n = (j) ? DanssPair.NNHits : DanssPair.NPHits;
			for (i=0; i<n; i++) if (HitArray[j].type[i].type == bSiPM) {
				side = HitArray[j].type[i].z & 1;
				z_PMT = HitArray[j].type[i].z / 20;
				xy_PMT = HitArray[j].type[i].xy / 5;
				PMTHits[side][z_PMT][xy_PMT].nHits++;
				PMTHits[side][z_PMT][xy_PMT].z = (HitArray[j].type[i].z - 20 * z_PMT) / 2;
				PMTHits[side][z_PMT][xy_PMT].xy = HitArray[j].type[i].xy - 5 * xy_PMT;
				PMTHits[side][z_PMT][xy_PMT].E = HitArray[j].E[i];	// we will use this number if it is a single number only
			}
	// Fill Ratio hist if the hit in PMT is single
			for (i=0; i<n; i++) if (HitArray[j].type[i].type == bPMT) {
				side = HitArray[j].type[i].z & 1;
				z_PMT = HitArray[j].type[i].z / 2;
				xy_PMT = HitArray[j].type[i].xy;
				if (PMTHits[side][z_PMT][xy_PMT].nHits == 1 && 
					PMTHits[side][z_PMT][xy_PMT].E > 1.5 && PMTHits[side][z_PMT][xy_PMT].E < 6) {
					h[side][z_PMT][xy_PMT][PMTHits[side][z_PMT][xy_PMT].z][PMTHits[side][z_PMT][xy_PMT].xy]->Fill(HitArray[j].E[i] / PMTHits[side][z_PMT][xy_PMT].E);
					hAvr[PMTHits[side][z_PMT][xy_PMT].z][PMTHits[side][z_PMT][xy_PMT].xy]->Fill(HitArray[j].E[i] / PMTHits[side][z_PMT][xy_PMT].E);
					if (side) {
