...
//	Ratios	Mid/Up
	for (i = 0; i < sizeof(PeriodList) / sizeof(PeriodList[0]); i++) {
		...
		sprintf(str, "Ratio Mid/Up %s;Positron energy, MeV;#frac{N_{DOWN}}{N_{UP}}", PeriodList[i].title);
		draw_single_ratio(strs, strm, strl, str, 0.6, 0.9);
		cv->Update();
		cv->Print(pname);
	}
...
//	after shutdown / before shutdown - use proper periods.
	draw_normalized_ratio(period_by_name("Beg5"), period_by_name("End4"), "hNRatioBeg5_2_End4", 
		"Normalized ratio after shutdown 5 / before shutdown 4;Positron energy, MeV", 0.9, 1.4, bgScale, Nscale);
	cv->Update();
	cv->Print(pname);

	draw_normalized_ratio(period_by_name("Beg6"), period_by_name("End5"), "hNRatioBeg6_2_End5", 
		"Normalized ratio after shutdown 6 / before shutdown 5;Positron energy, MeV", 0.9, 1.4, bgScale, Nscale);
	cv->Update();
	cv->Print(pname);
